...
module dpsk_stream_tx #(
    ...
    (* X_INTERFACE_PARAMETER = "ASSOCIATED_BUSIF m_axis:s_axis, ASSOCIATED_RESET rst_n, FREQ_HZ 250000000" *)
    input  wire         clk,
    ...
    // 基带映射与幅度控制
    // ------------------------------------------------------------------------
    // 只有在处于 TX 状态且是冲激时刻，才输出有效幅度；否则全输出 0 (实现突发静默)
    wire signed [15:0] active_i = (encoded_bit == 1'b0) ? 16'h7FFF : -16'h7FFF;
    wire signed [15:0] base_i   = (active_tx && is_impulse) ? active_i : 16'd0;
