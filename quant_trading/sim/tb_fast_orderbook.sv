...
module tb_fast_orderbook;
 ...
 wire[63:0]bram_wr;wire[31:0]bram_addr;wire[7:0]bram_we;
 fast_decoder_ip fd(.aclk(clk),.aresetn(rstn),.s_axi_ctrl_awaddr(8'd0),.s_axi_ctrl_awvalid(1'b0),.s_axi_ctrl_awready(),.s_axi_ctrl_wdata(32'd0),.s_axi_ctrl_wstrb(4'd0),.s_axi_ctrl_wvalid(1'b0),.s_axi_ctrl_wready(),.s_axi_ctrl_bresp(),.s_axi_ctrl_bvalid(),.s_axi_ctrl_bready(1'b1),.s_axi_ctrl_araddr(8'd0),.s_axi_ctrl_arvalid(1'b0),.s_axi_ctrl_arready(),.s_axi_ctrl_rdata(),.s_axi_ctrl_rresp(),.s_axi_ctrl_rvalid(),.s_axi_ctrl_rready(1'b1),.s_axis_raw_tdata(raw),.s_axis_raw_tvalid(raw_valid),.s_axis_raw_tready(raw_ready),.s_axis_raw_tlast(raw_last),.m_axis_decoded_tdata(decoded),.m_axis_decoded_tvalid(decoded_valid),.m_axis_decoded_tready(decoded_ready),.m_axis_decoded_tlast(decoded_last));
